...
## بازطراحیِ نهایی (design_handoff_final) — فازهای D1–D7 ✅

- **D1 خانهٔ v3** (گریدِ راستهٔ ۵ستونه + تب‌های فروشگاه/محصول + نشان‌شده‌ها + جدیدترین)
- **D2 rastehSearch** (تب‌های فروشگاه/محصول)
- **D3 shopDetail** (کاور + ۵ تب: محصولات/حراجی/ویترینو/نظرات/اطلاعات + گزارشِ تخلف)
- **D4 صفحهٔ محصول** (`listing`: گالری/قیمت/فروشنده/«دیدن در فروشگاه‌های دیگر»/نوارِ اکشنِ چسبان)
- **D5 compare** (مقایسهٔ فروشندگان)
- **D6 پنلِ فروشنده** (داشبوردِ آنالیتیکس + محصولات + سفارش‌ها + پیشنهادها)
- **D7 ادمین** (مدیریتِ راسته/محل + رسیدگی به گزارش‌ها)

دلتاهای سرور برای این فازها: `search/products?locationId`، `products/{id}/other-sellers`،
`reports`، و مدیریتِ راسته/محلِ ادمین (در ریپوی سرور).
